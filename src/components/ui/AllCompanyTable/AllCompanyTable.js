...
export default function App() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/get/companies").then((res) => {
  ...
  //       axios
  //       .delete(`http://localhost:5000/delete/company/${id}`)
  //       .then((res) => {
  ...
  const handleBlock = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch(`http://localhost:5000/company/block/${user._id}`).then((res) => {
          if (res.data.modifiedCount > 0) {
            axios.post("http://localhost:5000/block/email",{email: user?.companyEmail})
            .then(res=>{
...
const handleDeleted =(id) =>{
  ...
  return (
    <Table aria-label="Example static collection table">
      ...
      <TableBody>
        {
          allCompany?.map((item,index)=><TableRow key={index+1}>
          <TableCell>
          <Image className="" src={item?.companyLogo} height={70} width={70} alt="Profile image" />
...
<button onClick={() => handleBlock(item)}  title="Block"><MdBlock className="text-3xl" /></button>
                  )}
