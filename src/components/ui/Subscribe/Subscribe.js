...
const Subscribe = () => {
  ...
  return (
    <div className="text-center grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-4/5 mx-auto py-20">
      {subsInfo?.map((item) => (
        <div
          key={item.id}
          data-aos="zoom-out"
        ...
        >
          <h4 className="bg-lightPrimaryColor p-2 rounded  text-white ">
            {item.title}
          </h4>
          <p className="py-4">
          ...
          <div className="text-left h-[150px] items-center space-y-2 text-secondaryColor">
            <p className="flex items-center">
              <FaRightFromBracket className="mr-2" /> {item.disc1}
            </p>
            <p className="flex items-center">
